...
def validate_command_line_arguments() -> tuple:
    ...
    if not match:
        raise ValueError(f'Invalid URL. Expected URL to start with "{expected_core_url}", got "{url}"')

    if not file_name.endswith('.csv'):
        raise ValueError(f'Invalid file type. Expected .csv, got {os.path.splitext(file_name)[1]}')
...
def voter_turnout_data(city_url) -> list:
    ...
    registered_voters = [] #this line of code scrapes the number of registered voters in a particular city
    for i in city_url:
        response_2 = requests.get(i)
        doc_2 = BeautifulSoup(response_2.text, "html.parser")
        voters = [j.text.replace("\xa0", "") for j in doc_2.find_all("td", class_="cislo", headers="sa2")]
        registered_voters.extend(voters)

    ballot_papers = [] #this line of code scrapes the number of issued ballot papers in a particular city
    for i in city_url:
        response_2 = requests.get(i)
        doc_2 = BeautifulSoup(response_2.text, "html.parser")
        papers = [j.text.replace("\xa0", "") for j in doc_2.find_all("td", class_="cislo", headers="sa3")]
        ballot_papers.extend(papers)

    valid_votes = [] #this line of code scrapes the number of valid votes cast in election in a particular city
    for i in city_url:
        response_2 = requests.get(i)
        doc_2 = BeautifulSoup(response_2.text, "html.parser")
        votes = [j.text.replace("\xa0", "") for j in doc_2.find_all("td", class_="cislo", headers="sa6")]
...
def get_political_parties(city_url: list) -> list[list]:
    ...
    doc_2 = BeautifulSoup(response_2.text, "html.parser")
    table1 = [j.text.replace("\xa0", "") for j in doc_2.find_all("td", class_="overflow_name", headers="t1sa1 t1sb2")]
    table2 = [j.text.replace("\xa0", "") for j in doc_2.find_all("td", class_="overflow_name", headers="t2sa1 t2sb2")]
...
def get_votes(city_url: list) -> list:
    ...
    for i in city_url:
        response_2 = requests.get(i)
        doc_2 = BeautifulSoup(response_2.text, "html.parser")
        table1 = [j.text.replace("\xa0", "") for j in doc_2.find_all("td", class_="cislo", headers="t1sa2 t1sb3")]
...
def write_csv(file_name, city_list, data_collection, political_parties, total_votes) -> None:
    ...
    head = ['City Code', 'City Name', 'Registered Voters', 'Issued Ballots', 'Valid Votes']
    ...
    with open(file_name, "w", newline='') as file:
        ...
        for i in range(len(city_list[0])):
            writer.writerow([city_list[0][i], city_list[1][i]] + [data_collection[0][i]] + [data_collection[1][i]] + [data_collection[2][i]] + total_votes[i])
    print(f'Your file {file_name} has been successfully generated and stored in following directory:{file_path}')
...
:raises: ValueError: If there is an error in the process of program."""
    ...
    try:
        file = write_csv(file_name, city_list, data_collection, political_parties, total_votes)
    except ValueError as error:
        print(f'Error: {error}')
